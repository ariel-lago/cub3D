...
{
	static t_gnl_list	*lstline[OPEN_MAX] = {NULL};
	t_gnl_list			*piece;
	char				*line;
	int					err;
	...
	if (fd < 0 || fd >= OPEN_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	while (!ft_find_line(lstline[fd]))
	{
		piece = new_node_fd(fd, &err);
		if (!piece)
			break ;
		ft_lst_add_back(&lstline[fd], piece);
	}
	if (err == -1 || !lstline[fd] || ft_line_len(lstline[fd]) == 0)
	{
		ft_clean_sent_line(&lstline[fd]);
		lstline[fd] = NULL;
		return (NULL);
	}
	line = ft_lst_to_str(lstline[fd], ft_line_len(lstline[fd]));
	ft_clean_sent_line(&lstline[fd]);
	return (line);
}
